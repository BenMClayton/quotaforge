...
QuotaForge checks idle time throughout a run and pauses the active Codex process
when you return or the safe quota window closes. It preserves the work on a
dedicated local branch and resumes the same Codex session during the next
eligible window, even after a restart. It never redeems reset credits or spends
API credits. Every successful change is surrounded by an empty preflight commit
and a result commit, then pushed with ordinary Git credentials. Windows
notifications and an optional webhook explain what happened.
...
2. creates a dedicated local `quotaforge/wip/...` branch and an empty
   `preflight` commit;
3. asks a non-interactive, workspace-sandboxed Codex run to implement one small
   improvement and run relevant checks;
4. preserves that branch and Codex session if the user returns or the safe time
   window ends, prioritizing it at the next eligible opportunity;
5. blocks suspicious secret/key file paths;
6. commits the result, pushes both commits to the configured target branch, and
   removes the local work branch; and
7. sends a notification with the repository, summary, and commit ID.

Codex is instructed not to commit or push. QuotaForge owns those boundaries.
Continuation metadata lives under `%USERPROFILE%\.quotaforge\continuations`.
The target branch on GitHub is unchanged while an improvement is unfinished.
